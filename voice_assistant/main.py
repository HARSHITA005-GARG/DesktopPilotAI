...
def save_memory(memory_items):
    ...
    new_items = [item for item in memory_items[-MAX_MEMORY_ITEMS:] if item.lower() not in existing_keys]
    if not new_items:
        return
...
def extract_memory_candidates(transcript, reply):
    """Capture small durable facts worth remembering across sessions."""
    text = " ".join(part.strip() for part in [transcript, reply] if part).strip()
    lowered = text.lower()
    candidates = []
    ...
    if "remember this" in lowered or "remember that" in lowered:
        cleaned = re.sub(r".*remember (?:this|that)\s*", "", text, flags=re.IGNORECASE).strip(" .,!?:;")
